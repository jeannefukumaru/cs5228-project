...
mlflow.set_experiment('bbc_with_five_cats')
...
lfs = [lf_contains_tech_terms, lf_contains_business_terms, lf_contains_sport_terms, 
        lf_contains_entertainment_terms, lf_contains_politics_terms, lf_knn_pred_tech,
        lf_knn_pred_business, lf_knn_pred_sport, lf_knn_pred_entertainment, lf_knn_pred_politics]
...
bbc_stop_words = ['said', 'people', 'new', 'mr', 'quarter 2004', 'month 2004', 'year ago', 'year old', 'don know']
...
sklearn_model_weak_sup = LogisticRegressionCV(max_iter=500, cv=10, random_state=0, solver='liblinear').fit(X_train, preds_train_filtered)
sklearn_model_full_sup = LogisticRegressionCV(max_iter=500, cv=10, random_state=0, solver='liblinear').fit(X_train, y_train)
cv_weak = sklearn_model_weak_sup.score(X_train, y_train)
...
print_and_log_accuracies(cv_weak, cv_sup, dev_weak_sup_accuracy, dev_full_sup_accuracy)
